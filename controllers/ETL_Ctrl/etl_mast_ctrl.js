...
const ETL_mast_ctrl = (req, res, next) => {
    ...
    try {
        connectToOracle().then((connection) => {
            ...
            FROM etl_certificate_master WHERE rownum < = 100 AND folio_no=:folio_no AND security_code=:security_code
            `
            //
